...
export default function Cart({
  ...
  return (
    <div className='flex h-full flex-col items-center justify-start lg:gap-4'>
      ...
      <div className='m-10 h-full  w-10/12 overflow-hidden bg-pink-100'>
        <div className='flex h-full  flex-col justify-between overflow-hidden border  border-charcoal p-5 lg:p-10 '>
          {userCart.cart.products?.length ? (
          ...
          <div className='flex flex-col items-center justify-end  gap-3 border-t-[0.75px] border-charcoal/80 pt-5'>
            ...
            {userCart.cart.products?.length ? (
              <Link
                to={'/checkout'}
                onClick={() => setIsCartHidden(true)}
                className='rounded-sm bg-charcoal px-6  py-3 text-center  font-italiana text-sm text-white lg:px-10 xl:px-14 xl:text-xl'
              >
                PROCEED TO CHECKOUT
              </Link>
            ) : (
